...
export async function POST(req) {
    ...
    try {
        ...
        if (!validUser) return NextResponse.json({message: 'User not found' , success: false}, {status: 404});
        const validPassword = bcryptjs.compareSync(password, validUser.password);
        if (!validPassword) return NextResponse.json({message: 'Bad Credentials', success: false}, {status: 401});
