...
export default function QrCodePage({params}) {
    ...
    const addPoints = async (eventId, amount) => {
        try {
            const response = await fetch(`/api/events/attendance/${eventId}`, {
                ...
                }, body: JSON.stringify({userId: currentUser.userContent._id, amount: amount})
            });
    ...
    const checkAttended = async () => {
        ...
        return attended.includes(currentUser.userContent._id);
    }
    ...
    useEffect(() => {
        const checkAttendance = async () => {
            const attended = await checkAttended();
                ...
                <div className="flex justify-center items-center w-full md:max-w-md mx-auto py-20">
                    <div
                        ...
                        <div className="mb-0.5 space-y-3">
                            ...
                            <h2 className="text-center">You have already collected your points! Your attendance has been
                                accounted saved.</h2>
                        </div>
                    </div>
                </div>
